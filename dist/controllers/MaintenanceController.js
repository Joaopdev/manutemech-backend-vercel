...
const maintenanceDTOSchema_1 = require("../@types/dto/maintenanceDTOSchema");
...
class MaintenanceController {
    ...
    async createMaintenance(req, res) {
        const { entry_date, exits_date, km_vehicle, responsible_mechanic, maintenance_price, remarks, vehicleId, parts, services, workshopId, } = req.body;
        const userId = Number(req.userId);
        try {
            await maintenanceDTOSchema_1.maintenanceSchema.validate(req.body, { abortEarly: false });
            ...
            const maintenanceParts = await Promise.all(parts.map(async (part) => {
            ...
            if (error.inner && error.inner.length > 0) {
                const validationErrors = error.inner.map((validationError) => ({
    ...
    async getMaintenanceByVehicle(req, res) {
    ...
    async updateMaintenance(req, res) {
        ...
        try {
            ...
            const maintenancePartsPromises = parts.map(async (part) => {
                const maintenancePart = await prisma.maintenancePart.create({
                    data: {
